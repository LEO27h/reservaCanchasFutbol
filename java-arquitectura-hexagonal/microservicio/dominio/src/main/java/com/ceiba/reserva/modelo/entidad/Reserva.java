...
public class Reserva {
    ...
    private static final int ADICIONAR_RESERVA_ACTUAL = 1;
    ...
    public boolean aplicaDescuentoFutbolOcho(Long ocurrenciasPorUsuario) {
        boolean aplicaDescuento = false;
        aplicaDescuento = (ocurrenciasPorUsuario + ADICIONAR_RESERVA_ACTUAL) % CONDICION_RESERVAS_CONSECUTIVAS_PARA_DESCUENTO == 0 ? true : false;
    ...
    public void aplicarDescuento(Long cantidadOcurrenciasPorUsuario) {
        if (esCanchaFutbolOcho()) {
            if (aplicaDescuentoFutbolOcho(cantidadOcurrenciasPorUsuario)) {
